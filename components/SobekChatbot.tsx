...
const SobekChatbot: React.FC<SobekChatbotProps> = ({ isHidden = false }) => {
    ...
    if (isRoomIntent) {
      if (currentGuestId) {
        ...
        if (result.found && result.assignment) {
          ...
          setTimeout(() => {
            addBotMessage(
              `أنت منورنا يا ${(result.assignment!.personName && typeof result.assignment!.personName === 'string' ? result.assignment!.personName.split(' ')[0] : 'صديقي')}! دي بياناتك:`,
  ...
  const processGuestResult = (result: GuestResult): boolean => {
    if (result.found && result.assignment) {
      ...
      setTimeout(() => {
        addBotMessage(
          `أهلاً يا ${(result.assignment!.personName && typeof result.assignment!.personName === 'string' ? result.assignment!.personName.split(' ')[0] : 'يا جميل')}! لقيت مكانك:`,
          getContextualSuggestions(true),
          'room_result',
          { assignment: result.assignment, roommates: getRoommates(result.assignment!) }
        );
