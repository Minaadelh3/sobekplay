...
const MobileBottomNav: React.FC = () => {
  ...
  const navItems = [
    ...
    {
      name: 'Search', path: '#', action: () => alert('Search Coming Soon'), icon: (active: boolean) => (
    ...
    {
      name: 'My List', path: '/my-list', icon: (active: boolean) => (
        <svg className={`w-6 h-6 transition-colors ${active ? 'text-white' : 'text-zinc-500'}`} viewBox="0 0 24 24" fill={active ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={active ? 0 : 2}>
    ...
    {
      name: user ? 'Profile' : 'Login', path: user ? '#' : '/login', action: user ? () => { if (window.confirm('Log out?')) { supabase.auth.signOut(); navigate('/'); } } : undefined, icon: (active: boolean) => (
        <svg className={`w-6 h-6 transition-colors ${active ? 'text-white' : 'text-zinc-500'}`} viewBox="0 0 24 24" fill={active ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={active ? 0 : 2}>
