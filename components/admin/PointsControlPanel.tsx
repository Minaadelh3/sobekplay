...
const PointsControlPanel: React.FC<PointsControlPanelProps> = ({ targetId, targetName, targetType, currentPoints, onSuccess, onClose }) => {
    ...
    return (
        <div className="bg-[#1A1A1A] p-6 rounded-2xl border border-white/10 w-full max-w-md mx-auto shadow-2xl relative overflow-hidden">
            ...
            {/* Target Header */}
            <div className="bg-black/30 p-4 rounded-xl flex justify-between items-center mb-6 border border-white/5 relative z-10">
                <div>
            ...
            {/* Tabs */}
            <div className="flex gap-2 mb-6 border-b border-white/5 pb-1 relative z-10">
                <button onClick={() => setTab('ADJUST')} className={`pb-2 px-2 text-xs font-bold uppercase transition-colors ${tab === 'ADJUST' ? 'text-accent-gold border-b-2 border-accent-gold' : 'text-gray-500'}`}>Adjust</button>
                <button onClick={() => setTab('HISTORY')} className={`pb-2 px-2 text-xs font-bold uppercase transition-colors ${tab === 'HISTORY' ? 'text-accent-gold border-b-2 border-accent-gold' : 'text-gray-500'}`}>History</button>
                {targetType === 'TEAM' && (
                    <button onClick={() => setTab('LOCK')} className={`pb-2 px-2 text-xs font-bold uppercase transition-colors ${tab === 'LOCK' ? 'text-red-500 border-b-2 border-red-500' : 'text-gray-500'}`}>Lock / Freeze</button>
                )}
            </div>
            ...
            <div className="relative z-10 min-h-[300px]">
                {tab === 'ADJUST' && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        {/* <div>
                            <label className="block text-xs text-gray-400 mb-1 font-bold uppercase">Reason (Required)</label>
                            <textarea
                                value={reason}
                                onChange={(e) => setReason(e.target.value)}
                                placeholder="Audit trail verification..."
                                className="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:border-accent-gold outline-none h-20 resize-none"
                            />
                        </div> */}
                ...
                {tab === 'HISTORY' && (
                    <div className="animate-in fade-in slide-in-from-right-2 space-y-2 max-h-[350px] overflow-y-auto custom-scrollbar">
                        {loadingHistory ? <div className="text-center text-gray-500 py-8">Fetching records...</div> :
                            history.length === 0 ? <div className="text-center text-gray-500 py-8">No recent history found.</div> :
                                history.map((entry: any, i) => (
                                    <div key={i} className="bg-white/5 p-3 rounded-lg border border-white/5 flex justify-between items-center">
                                        <div>
                                            <div className="text-xs font-bold text-white mb-1">{entry.reason || 'Manual Adjustment'}</div>
                                        ...
                                        <div className={`font-mono font-bold ${entry.amount > 0 ? 'text-green-400' : 'text-red-400'}`}>
                ...
                {tab === 'LOCK' && (
                    <div className="animate-in fade-in slide-in-from-right-2 text-center py-8">
                        <div className={`w-20 h-20 rounded-full mx-auto flex items-center justify-center text-4xl mb-4 ${isLocked ? 'bg-red-500/20 text-red-500 animate-pulse' : 'bg-green-500/20 text-green-500'}`}>
                            {isLocked ? '🔒' : '🔓'}
                        </div>
                        <h3 className="text-xl font-bold text-white mb-2">{isLocked ? 'Scoring Locked' : 'Scoring Active'}</h3>
                        ...
                        {isLocked && (
                            <div className="mb-4">
                                <label className="block text-xs text-gray-400 mb-1">Update Reason</label>
                                <input
                                    value={lockReason} onChange={e => setLockReason(e.target.value)}
                                    className="bg-black/30 border border-white/10 p-2 rounded text-white text-center text-sm w-3/4"
                                />
                            </div>
                        )}
                        ...
                        <button
                            ...
                            className={`px-8 py-3 rounded-xl font-bold transition-all ${isLocked ? 'bg-white text-black' : 'bg-red-600 text-white'}`}
                        >
                            {isLocked ? '🔓 Unlock Scoring' : '🔒 Lock Scroring'}
                        </button>
                    </div>
                )}
            </div>
